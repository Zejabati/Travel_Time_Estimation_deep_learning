...
class EntireEstimator(nn.Module):
    ...
    def __init__(self, input_size, num_final_fcs, hidden_size=64):
        super(EntireEstimator, self).__init__()

        self.input2hid = nn.Linear(input_size, hidden_size)
        #print('size',input_size, hidden_size)
...
class Net(nn.Module):
    ...
    def init_weight(self):
        for name, param in self.named_parameters():
            if name.find('bias') != -1:
                ...
                # this if is only for transformer:
                if (param.data.dim() <2):
                    nn.init.constant(param.data, 1)
                else:
    ...
    def build(self):
        ...
        self.entire_estimate = EntireEstimator(input_size=self.spatio_temporal.out_size(
        ) + self.attr_net.out_size(), num_final_fcs=self.num_final_fcs, hidden_size=self.final_fc_size)
    ...
    def eval_on_batch(self, attr, traj, config):
        ...
        if self.training:
            ...
            mean, std = (self.kernel_size - 1) * \
                config['time_gap_mean'], (self.kernel_size - 1) * \
                config['time_gap_std']

            # get ground truth of each local path
            local_label = utils.get_local_seq(
                traj['time_gap'], self.kernel_size, mean, std)
